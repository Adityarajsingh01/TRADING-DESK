...
function renderMPTab() {
  ...
  if (rowNSr3) {
    ...
    for (let i=0;i<Math.min(mapSR3.length,23);i++) {
      ...
      if (pn!=null&&!isNaN(pn)) {
        const moStr = mapSR3[i].contract.replace('SR3 ','');
...
function renderSR3Tab() {
  ...
  for (let i=0;i<Math.min(map.length,23);i++) {
    ...
    // Extract month and year from contract label e.g. "SR3 Mar25" -> "Mar25"
...
function renderMasterTab() {
  ...
  if (rowN) {
    ...
    for (let i=0;i<Math.min(map.length,23);i++) {
      ...
      if (p==null||isNaN(p)) continue;
      const moStr = map[i].contract.replace('SR3 ','');
...
// Send current date to Streamlit parent for structures/notes sync
function notifyStreamlitDate(){
  try{
    var d = currentDate();
...
document.addEventListener('DOMContentLoaded',()=>{
  // Signal Streamlit component ready
